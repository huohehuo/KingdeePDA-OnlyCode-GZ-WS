if (exists (select * from sys.objects where name = 'proc_CheckBarCode'))
    drop proc proc_CheckBarCode
go
create proc proc_CheckBarCode
( 
...
update t_PDABarCodeCheckEntry set FQtying =FQtying+t.FQty,FCheckDate=CONVERT(varchar(128),GETDATE(),23),FCheckUserID=@FCheckUserID   from (select a.* from a_DetailsTable a left join t_PDABarCodeCheckEntry b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID and b.FID = @FID) as t where t_PDABarCodeCheckEntry.FBarCode=t.FBarCode and t_PDABarCodeCheckEntry.FID=@FID
