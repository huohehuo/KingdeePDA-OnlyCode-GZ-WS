if (exists (select * from sys.objects where name = 'proc_OtherOut'))
    drop proc proc_OtherOut
go
create proc proc_OtherOut
(
...
declare @FInterID varchar(20),     --单号id
        ...
        @FHeadSelfB0940 varchar(20),--是否良品
...
set @FUse =dbo.getString(@mainStr,'|',7)        --用途
set @FSManagerID=dbo.getString(@mainStr,'|',8) --发货
set @FFManagerID=dbo.getString(@mainStr,'|',9) --领料
...
set @FHeadSelfB0940=dbo.getString(@mainStr,'|',15)  --是否良品 
...
------------------------------------------------------------得到编号
set @FBillNo = ''
 exec proc_GetICBillNo 29, @FBillNo out 
-----------------------------------------------------------得到编号
...
FDeptID,Fuse,FCheckDate,FFManagerID,FSManagerID,FBillerID,FSupplyID,FBillTypeID,FMultiCheckDate1,FMultiCheckDate2,
FMultiCheckDate3,FMultiCheckDate4,FMultiCheckDate5,FMultiCheckDate6,FVchInterID,FRelateBrID,FPOOrdBillNo,FSelTranType,
FBrID,FManagerID,FEmpID,FManageType,FPrintCount,FObjectItem,FHeadSelfB0940,FHeadSelfB0939,FHeadSelfB0941) 
SELECT @FInterID,@FBillNo,'0',29,0,0,@value,@FROB,0,@Fdate,@FDeptID,@Fuse,@FCheckDate,@FFManagerID,@FSManagerID,@FBillerID,@FSupplyID,@FBillTypeID,Null,Null,Null,Null,Null,Null,0,0,'',@FSelTranType,0,@FManagerID,@FEmpID,0,0,0,@FHeadSelfB0940,CONVERT(varchar(128),GETDATE(),20),'PDA制单'
         ...
         select @FEntryID=isnull(MAX(FEntryID),0)+1 from ICStockBillEntry where FInterID=@FInterID 
             if exists(select 1 from ICStockBillEntry where FItemID=@FItemID and FBatchNo=@FBatchNo and FUnitID=@FUnitID and isnull(FKFDate,'')=isnull(@FKFDate,'') and FKFPeriod=@FKFPeriod and FDCStockID=@FDCStockID and FDCSPID=@FDCSPID and FInterID =@FInterID)
		       begin
		        ...
		        begin
INSERT INTO ICStockBillEntry (FInterID,FEntryID,FBrNo,FMapNumber,FMapName,FItemID,FAuxPropID,FBatchNo,FQty,FUnitID,Fauxqty,FSecCoefficient,FSecQty,
FAuxPlanPrice,FPlanAmount,Fauxprice,Famount,Fnote,FKFDate,FKFPeriod,FPeriodDate,FIsVMI,FEntrySupply,FDCStockID,FDCSPID,FSNListID,FSourceBillNo,FSourceTranType,
FSourceInterId,FSourceEntryID,FOrderBillNo,FOrderInterID,FOrderEntryID,FPlanMode,FMTONo,FSEOutBillNo,FSEOutEntryID,FSEOutInterID)  
SELECT @FInterID,@FEntryID,'0','','',@FItemID,0,@FBatchNo,@FQty,@FUnitID,@Fauxqty,@FSecCoefficient,@FSecQty,@FAuxPlanPrice,@FPlanAmount,@Fauxprice,@Famount,'',@FKFDate,@FKFPeriod,@FPeriodDate,0,0,@FDCStockID,@FDCSPID,0,'',0,0,0,'',0,0,14036,'','',0,0 
end
...
EXEC p_UpdateBillRelateData 29,@FInterID,'ICStockBill','ICStockBillEntry' 
...
begin
--单个
declare @FIsOutStore varchar(128)
set @FIsOutStore = '已出库'
if(@FROB=-1)
begin
set @FIsOutStore = '未出库'
insert into t_PDABarCodeSign_Out(FInterID,FBarCode,FQty,FBillNo,FRemark,FInterID_Before,FDateOutStore_Before,FUserOutStore_Before,FStatus,FDateUpLoad,FIsInStore,FStockID,FStockPlaceID)select @FInterID,a.FBarCode,a.FQty,@FBillNo,'其他出库单红字',b.FInterIDOut,b.FDateOutStore,b.FUserOutStore,1,convert(varchar(20),GETDATE(),20),ISNULL(b.FIsInStore,'未入库'),b.FStockID,b.FStockPlaceID from a_DetailsTable  a inner join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID
end
else
begin
insert into t_PDABarCodeSign_Out(FInterID,FBarCode,FQty,FBillNo,FRemark,FInterID_Before,FDateOutStore_Before,FUserOutStore_Before,FStatus,FDateUpLoad,FStockID,FStockPlaceID)select @FInterID,a.FBarCode,a.FQty,@FBillNo,'其他出库单蓝字',b.FInterIDOut,b.FDateOutStore,b.FUserOutStore,1,convert(varchar(20),GETDATE(),20),b.FStockID,b.FStockPlaceID from a_DetailsTable  a inner join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID
end
update t_PDABarCodeSign  set FInterIDOut=@FInterID,FDateOutStore=@fdate,FIsOutStore=@FIsOutStore,FUserInStore=@FBillerID,FQtyOut =FQtyOut + t.FQty,FStockID=t.FStockID,FStockPlaceID=t.FStockPlaceID  from (select a.* from a_DetailsTable a left join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID and ISNULL( b.FIsInStore,'已入库')='已入库') as t where t_PDABarCodeSign.FBarCode=t.FBarCode and ISNULL( t_PDABarCodeSign.FIsInStore,'已入库')='已入库'
update t_PDABarCodeSign  set  FQtyOut =  0,FIsInStore='已入库',FRemark1='其他出库单红字',FDateInStore=@fdate,FUserInStore=@FBillerID  from (select a.* from a_DetailsTable a left join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID and ISNULL( b.FIsInStore,'未入库')='未入库') as t where t_PDABarCodeSign.FBarCode=t.FBarCode and ISNULL( t_PDABarCodeSign.FIsInStore,'未入库')='未入库'
