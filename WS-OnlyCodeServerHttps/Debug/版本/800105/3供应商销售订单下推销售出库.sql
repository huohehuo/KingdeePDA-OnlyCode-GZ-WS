...
begin
--单个
insert into t_PDABarCodeSign_Out(FInterID,FBarCode,FQty,FBillNo,FRemark,FInterID_Before,FDateOutStore_Before,FUserOutStore_Before,FStatus,FDateUpLoad,FTypeID,FPrice,FStockID,FStockPlaceID,FMDUserID)select @FInterID,a.FBarCode,a.FQty,@FBillNo,'供应商销售订单下推销售出库',b.FInterIDOut,b.FDateOutStore,b.FUserOutStore,1,convert(varchar(20),GETDATE(),20),21,a.FPrice,b.FStockID,b.FStockPlaceID,@FUserID from a_DetailsTable  a inner join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID
