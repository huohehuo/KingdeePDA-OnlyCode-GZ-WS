 ...
 *          销售出库红字：条码检测与写入临时表
 */
@WebServlet(urlPatterns = "/CodeCheckAndInsert4RedIO")
public class CodeCheckAndInsert4RedIO extends HttpServlet {
...
//        Lg.e("列表",testBS);
            try {
                ...
                if ("OK".equals(backString)){
                        backString = "";
                        conn = JDBCUtil.getConn4Web();
                        sta2 = conn.prepareStatement("exec proc_SupplyOutStoreBarCodeRed_Insert ?,?,?,?,?");
                        sta2.setString(1, "0");
                        sta2.setString(2, imie);
                        sta2.setString(3, barcode);
                        sta2.setString(4, "-"+qty);
                        sta2.setString(5, userid);
