 ...
 *           扫码查询：条码检测与写入临时表
 */
@WebServlet(urlPatterns = "/CodeCheckAndInsert4ScanIO")
public class CodeCheckAndInsert4ScanIO extends HttpServlet {
...
//        Lg.e("列表",testBS);
            try {
                ...
                sta = conn.prepareStatement("exec proc_BarCodeSignScan_check ?");
...
//                        qty = rs.getString("FQty");
                ...
                if ("OK".equals(backString)){
                        backString = "";
                        conn = JDBCUtil.getConn4Web();
                        sta2 = conn.prepareStatement("exec proc_BarCodeSignScan_Insert ?,?,?");
                        sta2.setString(1, "0");
