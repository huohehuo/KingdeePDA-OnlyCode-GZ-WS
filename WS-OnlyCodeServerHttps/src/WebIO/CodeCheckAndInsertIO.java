 ...
 *           销售出库：条码检测与写入临时表
 */
@WebServlet(urlPatterns = "/CodeCheckAndInsertIO")
public class CodeCheckAndInsertIO extends HttpServlet {
...
//        Lg.e("列表",testBS);
            try {
                ...
                if ("OK".equals(backString)){
                        backString = "";
                        conn = JDBCUtil.getConn4Web();
                        sta2 = conn.prepareStatement("exec proc_SupplyOutStoreBarCode_Insert ?,?,?,?,?");
                        sta2.setString(1, "0");
