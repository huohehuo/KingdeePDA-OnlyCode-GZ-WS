 ...
 */
@WebServlet(urlPatterns = "/StartTask")
        ...
        if(json!=null&&!json.equals("")){
            try {
                ...
                System.out.println(request.getParameter("sqlip")+" "
                        +request.getParameter("sqlport")+" "+request.getParameter("sqlname")
                        +" "+request.getParameter("sqlpass")+" "+request.getParameter("sqluser"));
                String SQL = "Update ICMO SET FStartFlag=1 Where FInterID=? and FStatus=1 and FStartFlag=0 and FSuspend=0 and FCancellation=0";
                ...
                if(i>0){
                    ...
                    writer.write(CommonJson.getCommonJson(false,"生产任务单开工失败，只能对下达或打印状态的单据执行开工"));
                }
