 ...
 */
@WebServlet("/SCRWDPDSCHBDUpload")
public class SCRWDPDSCHBDUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public SCRWDPDSCHBDUpload() {
        super();
		...
		if(parameter!=null&&!parameter.equals("")){
			try {
				...
				for(int i =0;i<pBean.list.size();i++){
					sta = conn.prepareStatement("exec proc_ICMOReport ?,?,?,?,?,?");
