 ...
 */
@WebServlet("/CodeCheckInsert4Bulk")
public class CodeCheckInsert4Bulk extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public CodeCheckInsert4Bulk() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(parameter!=null&&!parameter.equals("")){
			try {
				...
				sta = conn.prepareStatement("exec proc_LooseBarCode_Insert ?,?,?,?,?,?,?,?,?,?");

					sta.setString(1, bean.FOrderID);
					sta.setString(2, bean.FPDAID);
					sta.setString(3, bean.FBarCode);
					sta.setString(4, bean.FItemID);
					sta.setString(5, bean.FUnitID);
					sta.setString(6, bean.FStockID);
					sta.setString(7, bean.FStockPlaceID);
					sta.setString(8, bean.FBatch);
					sta.setString(9, bean.FInStore);
					sta.setString(10, bean.FOutStore);
					rs = sta.executeQuery();
					if (rs != null) {
						while (rs.next()) {
