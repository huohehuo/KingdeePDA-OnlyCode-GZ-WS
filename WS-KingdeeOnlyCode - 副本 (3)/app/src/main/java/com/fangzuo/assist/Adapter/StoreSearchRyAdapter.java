 ...
 */
//代码，名称，规格，总库存，单位
public class StoreSearchRyAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    ...
    public StoreSearchRyAdapter(Context context, ArrayList<CheckLogBackBean> items) {
        this.items = items;
...
//        if(1 == viewType){
            View v = mInflater.inflate(R.layout.item_store_search,parent,false);
...
//        int pos;
            ((MarkHolder) holder).billno.setText(   "物料编码:"+items.get(position).getFNumber());
//            ((MarkHolder) holder).name.setText(   items.get(position).getFName());
            ((MarkHolder) holder).clienName.setText(   "物料名称:"+items.get(position).getFName());
            ((MarkHolder) holder).num.setText(   "数量:"+items.get(position).getFBaseQty());
            ((MarkHolder) holder).price.setText(    "仓库:"+items.get(position).getFStorage());
...
//                    }
    ...
    public void setOnItemClickListener(StoreSearchRyAdapter.OnItemClickListener listener) {
