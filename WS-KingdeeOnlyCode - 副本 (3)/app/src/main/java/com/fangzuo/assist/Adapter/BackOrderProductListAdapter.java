...
public class BackOrderProductListAdapter extends BaseAdapter {
    Context context;
    List<BackOrderResult.TradeReturnsBean.DetailsBean> items;

    public BackOrderProductListAdapter(Context context, List<BackOrderResult.TradeReturnsBean.DetailsBean> items) {
...
//        viewHolder.billNo.setText("订单编号:"+items.get(i).getCode());
        viewHolder.tvName.setText("商品名称:"+items.get(i).getItem_name());
        viewHolder.tvNumber.setText("商品代码:"+items.get(i).getItem_code());
        viewHolder.tvModel.setText("规格型号:"+items.get(i).getSku_note()==null?"无":items.get(i).getSku_note());
        viewHolder.tvNum.setText("数量:"+items.get(i).getQty());
        String num = DataModel.getDetailNum(context,items.get(i).getItem_code(),"");
        viewHolder.tvNumIng.setText("已验数量:"+num);
        if((int) (Double.parseDouble(num)/items.get(i).getQty()*100)==100){
...
//        viewHolder.unit.setText("单位:"+unit.FName);
        viewHolder.pg.setProgress((int) (MathUtil.toD(num)/items.get(i).getQty()*100));
        return view;
    }
