 ...
 */
//代码，名称，规格，总库存，单位
public class LogSearchRyAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    ...
    public LogSearchRyAdapter(Context context, ArrayList<CheckLogBackBean> items) {
        this.items = items;
...
//        if(1 == viewType){
            View v = mInflater.inflate(R.layout.item_log_search,parent,false);
...
//            ((MarkHolder) holder).name.setText(   items.get(position).getFName());
            ...
            ((MarkHolder) holder).price.setText(    "总价:"+items.get(position).getFPrice());
...
//                    }
    ...
    public void setOnItemClickListener(LogSearchRyAdapter.OnItemClickListener listener) {
