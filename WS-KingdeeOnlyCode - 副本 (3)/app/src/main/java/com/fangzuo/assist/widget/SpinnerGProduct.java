...
public class SpinnerGProduct extends RelativeLayout {
    // 返回按钮控件
    private Spinner mSp;
    // 标题Tv
    ...
    private ArrayList<GProduct> container;
    private GProductSpAdapter adapter;
    ...
    public SpinnerGProduct(Context context, AttributeSet attributeSet) {
        ...
        adapter = new GProductSpAdapter(context, container);
...
//                        payTypeDao.detachAll();
                    if (null!=dBean.gProducts && dBean.gProducts.size() > 0 && container.size()<=0){
                        container.addAll(dBean.gProducts);
...
//        else {
        GProductDao employeeDao = daoSession.getGProductDao();
        List<GProduct> employees = employeeDao.loadAll();
...
//        }
        ...
        mSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                GProduct employee = (GProduct) adapter.getItem(i);
...
//    }
    ...
    public void setAutoSelection(String saveKeyStr,String string) {
        ...
        for (int j = 0; j < adapter.getCount(); j++) {
            if (((GProduct) adapter.getItem(j)).FName.equals(autoString)
                    || ((GProduct) adapter.getItem(j)).FInterID.equals(autoString)) {
                mSp.setSelection(j);
//                autoString = null;
                break;
            }
    ...
    public GProductSpAdapter getAdapter() {
        return adapter;
    }
