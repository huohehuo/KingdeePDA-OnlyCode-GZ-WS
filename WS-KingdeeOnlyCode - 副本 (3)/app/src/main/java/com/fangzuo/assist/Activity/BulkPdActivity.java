...
import com.fangzuo.assist.databinding.ActivityBulkPdBinding;
...
public class BulkPdActivity extends BaseActivity {
    @BindView(R.id.ed_storage)
    EditText edStorage;
    @BindView(R.id.iv_storage)
    ImageView ivStorage;
    private ActivityBulkPdBinding binding;
...
//                getProduct();
            ...
            case EventBusInfoCode.Upload_OK:
                ...
                ordercode++;
                share.setOrderCode(BulkPdActivity.this, ordercode);
                reCheckUi();
                LoadingUtil.dismiss();
                break;
    ...
    protected void initView() {
        binding = DataBindingUtil.setContentView(this, R.layout.activity_bulk_pd);
        ButterKnife.bind(this);
        binding.activityTop.tvTopTitle.setText("散装盘点单");
...
//        binding.spWhichStorage.setAutoSelection(Config.Storage+activity, "",false);
        reCheckUi();
    }
...
//            DataModel.codeCheck(WebApi.CodeCheck4Bulk,gson.toJson(bean));
//        }else{
//            lockScan(0);
...
//
    ...
    private void CheckCode() {
        LoadingUtil.showDialog(mContext, "正在检测...");
        //查询条码唯一表
        CodeCheckBean bean = new CodeCheckBean(binding.edSn.getText().toString(), binding.etZz.getText().toString() + "", BasicShareUtil.getInstance(mContext).getIMIE());
//            bean.FScanType="1";
//            DataModel.codeCheck(gson.toJson(bean));
        DataModel.codeCheck(WebApi.CodeCheck4Bulk, gson.toJson(bean));
    }
...
//            return;
//        }
        if (!binding.cbOutstore.isChecked() && !binding.cbInstore.isChecked() && !binding.cbInstoreNo.isChecked()) {
            Toast.showText(mContext, "请选择出入库状态");
            MediaPlayer.getInstance(mContext).error();
            lockScan(0);
            return;
        }
...
//        CodeCheckBean bean = new CodeCheckBean(barcodeStr,binding.etZz.getText().toString() + "",BasicShareUtil.getInstance(mContext).getIMIE());
        ...
        bean.FBarCode = binding.edSn.getText().toString();
        bean.FOrderID = ordercode + "";
        bean.FPDAID = BasicShareUtil.getInstance(mContext).getIMIE();
        bean.FItemID = product.FItemID;
        bean.FUnitID = unit.FMeasureUnitID;
        bean.FStockID = binding.cbInstore.isChecked() ? storage.FItemID : "0";
        bean.FStockPlaceID = binding.cbInstore.isChecked() ? waveHouse == null ? "0" : waveHouse.FSPID : "0";
        bean.FBatch = binding.tvPihao.getText().toString();
        bean.FInStore = binding.cbInstore.isChecked() ? "已入库" : "未入库";
        bean.FOutStore = binding.cbOutstore.isChecked() ? "已出库" : "未出库";
        DataModel.codeInsert(WebApi.CodeCheckInsert4Bulk, gson.toJson(bean));
    }

    private void Addorder() {
        List<T_Detail> list = t_detailDao.queryBuilder().where(
                T_DetailDao.Properties.FBarcode.eq(binding.edSn.getText().toString())
        ).build().list();
        if (list.size() > 0) {
            return;
        }
        ...
        t_main.DataPush = binding.edSn.getText().toString();
        t_main.IMIE = getIMIE();
        t_main.activity = activity;
        t_main.orderId = ordercode;
        t_main.Rem = ordercode + "";
        long insert1 = t_mainDao.insert(t_main);

        T_Detail t_detail = new T_Detail();
        t_detail.FIndex = second;
        t_detail.FBarcode = binding.edSn.getText().toString();
...
//                Log.e("detail", detail);
        ...
        pBean.list = data;
        DataModel.upload(mContext, getBaseUrl() + WebApi.BulkUpload, gson.toJson(pBean));
    }
