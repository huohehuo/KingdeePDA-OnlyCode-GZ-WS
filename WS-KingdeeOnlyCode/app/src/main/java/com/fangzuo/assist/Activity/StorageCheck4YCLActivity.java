...
public class StorageCheck4YCLActivity extends BaseActivity {
    ...
    public void initView() {
        setContentView(R.layout.activity_storage_check4_ycl);
        mContext = this;
...
//        }
    ...
    private void getDate(String productID) {
        App.getRService().doIOAction(WebApi.SEARCHPRODUCTS,productID, new MySubscribe<CommonResponse>() {
//        App.getRService().getProductForId(productID, new MySubscribe<CommonResponse>() {
            @Override
...
//        unitAdapter = CommonMethod.getMethod(mContext).getUnitAdapter(product.FUnitGroupID, spUnit);
    ...
    private void getInstorageNumList(boolean type) {
        ...
        String json;
        String productId="";
        if (etStorage.getText().toString().equals("")){
            storage = null;
        }
        if (type){
            ...
            iBean.FBatchNo = edPihao.getText().toString();
...
//    }
    ...
    @OnClick({R.id.btn_clear,R.id.btn_clear2, R.id.btn_check_by,R.id.check,R.id.check_storage})
