...
////                break;
            case EventBusInfoCode.CodeCheck_OK://检测条码成功
                ...
                Lg.e("条码检查："+codeCheckBackDataBean.toString());
                    ...
                    LoadingUtil.dismiss();
                if ("OK".equals(codeCheckBackDataBean.FTip)){
                    ScanBarCode(codeCheckBackDataBean.FItemID);
                }else{
                    lockScan(0);
                    String tips=null;
                    if ("OK1".equals(codeCheckBackDataBean.FTip))tips="该箱码存在出库退回记录,是否继续添加";
                    if ("OK2".equals(codeCheckBackDataBean.FTip))tips="该箱码存在返修记录,是否继续添加";
                    if ("OK3".equals(codeCheckBackDataBean.FTip))tips="该条码存在出库退回记录,是否继续添加";
                    if ("OK4".equals(codeCheckBackDataBean.FTip))tips="该条码存在返修记录,是否继续添加";
                    AlertDialog.Builder ab = new AlertDialog.Builder(mContext);
                    ab.setTitle(tips==null?"":tips);
                    ab.setPositiveButton("添加", new DialogInterface.OnClickListener() {
                        @Override
...
//                    batchOk = false;
                    ...
                    if (null!=pushDownSub && "成品".equals(pushDownSub.FProductType)){
                        //插入条码唯一临时表
...
//                            PushDownSub pushDownSub = (PushDownSub) pushDownSubListAdapter.getItem(i);
                        //条码的单位和仓库不可能为空，只有明细的仓库可能为空
                        Lg.e("setProductUnit",default_unitID);
...
//                        break;
    ...
    protected void initData() {
        ...
        if (list1.size() > 0) {
            ...
            tvPhoneAddress.setText(list1.get(0).FPhoneAddress);
            mainIfCheck = list1.get(0).FIfCheck;
//            ordercode = Long.parseLong(list1.get(0).FInterID);
        }
        ordercode = DataModel.findOrderCode(mContext,activity,fidcontainer);
        edKdno.setText(Hawk.get(Config.saveKdNo+ordercode,""));
...
//        });
    ...
    private void clickList(final Product product) {
        ...
        if ("成品".equals(pushDownSub.FProductType)){
            ...
            wavehouseAutoString=code4Plan?product.FSPID:(codeCheckBackDataBean==null?"":codeCheckBackDataBean.FStockPlaceID);
...
//            setfocus(edNum);
                ...
                bean.FIfCheck = mainIfCheck;
                DataModel.codeCheckForOut4PdSN(gson.toJson(bean));
                canClick =true;
            }
        }
    ...
    private void LoadBasicData() {
        ...
        spCapture.setAutoSelection(Config.People2+activity,"");
        spSendman.setAutoSelection(Config.People3+activity,"");
...
//        spSendMethod.setSelection(share.getPDSNSendMethod());
    ...
    private void AddorderBefore() {
        ...
        if (null!=pushDownSub.FDCStockID && "成品".equals(pushDownSub.FProductType) && !default_storageID.equals(pushDownSub.FDCStockID)){
            MediaPlayer.getInstance(mContext).error();
            Toast.showText(mContext, "条码仓库与通知单的仓库不一致");
            lockScan(0);
            return;
        }
        if (fid == null) {
...
//                    Log.e("captureID", captureID == null ? "" : captureID);
        ...
        t_detail.FOrderId = ordercode;
                    ...
                    t_detail.FRate = edKdno.getText().toString();//快递单号
...
//                    pici.notifyDataSetChanged();
    ...
    private void resetAll() {
        ...
        product = null;
        codeCheckBackDataBean = null;
        Hawk.put(Config.saveKdNo+ordercode,edKdno.getText().toString());
    }
