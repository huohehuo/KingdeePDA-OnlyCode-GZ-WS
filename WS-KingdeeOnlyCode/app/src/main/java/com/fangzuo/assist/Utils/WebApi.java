package com.fangzuo.assist.Utils;

/**
 * Created by NB on 2017/7/24.
 */

public class WebApi {
    public static final String OCUPLOAD = "PushDownOCUpload";

    public static final String SCRWSCLLUpload = "SCRWSCLLUpload";
    public static final String SCRWDPDSCHBDUPLOAD = "SCRWDPDSCHBDUpload";
    public static final String XSDDPDFLTZUPLOAD = "XSDDPDFLTZDUpload";
    public static final String SCANTODLPDLIST = "ScanToDLPDlist";
    public static final String PUSHDOWNPPISUPLOAD = "PushDownPPISUpload";
    public static final String CONNECTSQL = "ConnectDataBase";
    public static final String PROP = "SetPropties";
    public static final String DOWNLOADDATA = "DownloadInfo";
    public static final String UPLOADPIS = "PurchaseInStoreUpload";
    public static final String UPLOADPROIS = "ProductInStoreUpload";
    public static final String UPLOADPO = "PurchaseOrderUpload";
    public static final String UPLOADSO = "SalesOrderUpload";
    public static final String UPLOADSOUT = "SoldOutUpload";
    public static final String OTHERINSTORE = "OtherInStoreUpload";
    public static final String OTHEROUTSTORE = "OtherOutStoreUpload";
    public static final String DBUPLOAD = "DBUpload";
    public static final String PDMAINLIST = "GetPDList";
    public static final String PDSUBLIST = "GetPDSubList";
    public static final String PushDownOCOSUpload = "PushDownOCOSUpload";
    public static final String PushDownOCISUpload = "PushDownOCISUpload";
    public static final String PDUPLOAD = "PDUpload";
    public static final String PD2UPLOAD = "PD2UPLOAD";
    public static final String PUSHDOWNLIST = "PushDownList";
    public static final String PUSHDOWNDLLIST = "PushDownloadList";
    public static final String PUSHDOWNSOUPLOAD = "PushDownSOUpload";
    public static final String PUSHDOWNSLUPLOAD = "PushDownSLUpload";
    public static final String PUSHDOWNPOUPLOAD = "PushDownPOUpload";
    public static final String SCRWDPDCPRKUpload = "SCRWDPDCPRKUpload";
    public static final String HBDPDCPRKUpload = "HBDPDCPRKUpload";
    public static final String CGDDPDSLTZDUpload = "CGDDPDSLTZDUpload";
    public static final String PUSHDOWNSNUPLOAD = "PushDownSNUpload";
    public static final String PdBackMsg2SaleOutUpload = "PdBackMsg2SaleOutUpload";
    public static final String SEARCHPRODUCTS = "SearchProducts";
    public static final String PRODUCTSEARCHLIKE = "ProductSearchLike";
    public static final String ManSearchLike = "ManSearchLike";
    public static final String GETINSTORENUM = "GetInStoreNum";
    public static final String GETNEWVERSION = "NewVersion";
    public static final String REGISTER = "Register";
    public static final String SETTIMEUSE = "SetLastUseDate";
    public static final String GETPICI = "GetPici";
    public static final String TESTSERVLET = "TestServlet";
    public static final String TESTDATABASE = "TestDataBase";
    public static final String PRPDUCTSEARCHWHERE = "ProductSearchWhere";
    public static final String PRODUCEANDGET = "ProduceAndGetUpload";

    public static final String SearchDbType = "SearchDbType";
    public static final String SearchStorage = "SearchStorage";
    public static final String SUPPLIERSEARCHLIKE = "SupplerSearchLike";
    public static final String CLIENTSEARCHLIKE = "ClientSearchLike";
    public static final String SEARCHJHSEARCHLIKE = "jhSearchLike";
    public static final String PackageUpload = "PackageUpload";
    public static final String BulkUpload = "BulkUpload";


    public static final String GetInStorageNumList = "GetInStorageNumList";
    public static final String CodeStateCheck = "CodeStateCheck";



    public static final String DownloadInfoForFile = "DownloadInfoForFile";
    public static final String CodeCheck = "CodeCheck";
    public static final String CodeCheckOut4PdSn = "CodeCheckOut4PdSn";
    public static final String DeleteForOnlyCode = "DeleteForOnlyCode";
    public static final String InsertForOnlyCodeIn = "InsertForOnlyCodeIn";
    public static final String InsertForOnlyCodeOut = "InsertForOnlyCodeOut";
    public static final String InsertForOnlyCodeOutRed = "InsertForOnlyCodeOutRed";
    public static final String CheckCode = "CheckCode";
    public static final String CheckCodeOut = "CheckCodeOut";
    public static final String GyUpload = "GyUpload";
    public static final String GyRedUpload = "GyRedUpload";
    public static final String CheckFirstInOut = "CheckFirstInOut";
    public static final String Checkcode4OutRed = "Checkcode4OutRed";
    public static final String CodeCheckForOutForRed = "CodeCheckForOutForRed";
    public static final String CodeInsert4OutRed = "CodeInsert4OutRed";
    public static final String CodeCheck4Bulk = "CodeCheck4Bulk";
    public static final String CodeCheckInsert4Bulk = "CodeCheckInsert4Bulk";
    public static final String CheckSearchNo = "CheckSearchNo";
    public static final String getSendOrderList = "getSendOrderList";
    public static final String getSendOrderRedList = "getSendOrderRedList";
    public static final String InsertForPd = "InsertForPd";

    public static final String ChangePriceUpload = "ChangePriceUpload";
    //p2
    public static final String CodeCheck4Saleout = "CodeCheck4Saleout";
    public static final String CodeInsert4Saleout = "CodeInsert4Saleout";
    public static final String CodeCheck4SaleoutRed = "CodeCheck4SaleoutRed";
    public static final String CodeInsert4SaleoutRed = "CodeInsert4SaleoutRed";
    public static final String CodeCheck4ScanCheck = "CodeCheck4ScanCheck";
    public static final String CodeInsert4ScanCheck = "CodeInsert4ScanCheck";
    public static final String ScanCheckUpload = "ScanCheckUpload";
    public static final String CreateSOUpload = "CreateSOUpload";
    public static final String CreateSORedUpload = "CreateSORedUpload";
    public static final String CheckLogSearch1 = "CheckLogSearch1";
    public static final String CheckLogSearch2 = "CheckLogSearch2";
    public static final String CheckStoreSearch1 = "CheckStoreSearch1";
    public static final String CheckStoreSearch2 = "CheckStoreSearch2";

}
