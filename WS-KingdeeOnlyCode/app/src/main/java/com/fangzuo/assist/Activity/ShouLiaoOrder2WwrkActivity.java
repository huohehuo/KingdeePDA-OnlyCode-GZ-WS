...
public class ShouLiaoOrder2WwrkActivity extends BaseActivity {
    private int tag =31;
    private int activity = Config.ShouLiaoOrder2WwrkActivity;
...
//                LoadingUtil.dismiss();
    ...
    protected void initView() {
        setContentView(R.layout.activity_shouliao_order2_wwrk);
...
//                    ).build().list();
    ...
    private void postToServer(ArrayList<PurchaseInStoreUploadBean.purchaseInStore> data) {
        ...
        pBean.list = data;
        Gson gson = new Gson();
        App.getRService().doIOAction(WebApi.ShouLiaoOrder2WwrkActivityUpload, gson.toJson(pBean), new MySubscribe<CommonResponse>() {
            @Override
            public void onNext(CommonResponse commonResponse) {
    ...
    public static void start(Context context, String code, ArrayList<String> fid) {
        Intent starter = new Intent(context, ShouLiaoOrder2WwrkActivity.class);
