if (exists (select * from sys.objects where name = 'proc_InStoreBarCode_Red_check'))
    drop proc proc_InStoreBarCode_Red_check
go
create proc proc_InStoreBarCode_Red_check
(  
	  ...
	  begin
		  ...
		  if(@FIsInStore='已入库')
		   begin
		    ...
		    if(@FQtyAll=0)
		    begin
		     set @FBillNo='该箱码已完全出库,不能再次红字入库'
		    end
		   end 
		  else 
	        begin
	        set @FBillNo='未入库的箱码,不能红字入库'
	        end 
	   end 
  ...
  begin
         ...
         begin
			  ...
			  begin
				...
				if(@FQtyAll=0)
				begin
				set @FBillNo='该条码已完全出库,不能再次红字入库'
				end
			  end
			  else
			  begin
				set @FBillNo='未入库的条码不能红字入库'
			  end
	     end
	     else
	     begin
	         set @FBillNo= '该条码已发生业务关系,暂时不能红字入库'
	     end
  end
  else
  begin
