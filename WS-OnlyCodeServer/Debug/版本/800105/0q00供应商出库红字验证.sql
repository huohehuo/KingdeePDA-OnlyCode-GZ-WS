if (exists (select * from sys.objects where name = 'proc_SupplyOutStoreBarCodeRed_check'))
    drop proc proc_SupplyOutStoreBarCodeRed_check
go
create proc proc_SupplyOutStoreBarCodeRed_check
(  
...
declare @FBillNo varchar(128),
        @FInterIDAssemble int,--是否装箱
          ...
          @FOLOrderBillNo varchar(255),--网上订单号
...
set @FBillNo='OK'
  ...
  begin
         ...
         begin
			  ...
			  if(@FIsInStore='已入库' and @FIsOutStore = '未出库')
			  begin
					set @FBillNo='已入库未出库的条码不能红字出库'
			  end
			  else
			  begin
			    if(@FIsInStore='已入库')
 ...
 if(@FBillNo='OK')
 begin
    if not exists(select 1 from t_PDABarCodeSign_Out  t1 inner join ICStockBill t2 on t1.FInterID=t2.FInterID where t1.FBarCode=@FBarCode and t1.FTypeID=21)
     set @FBillNo = '该条码不是供应商端出库,不能做供应商端红字出库'
    else
    select top 1 @FOLOrderBillNo=FOLOrderBillNo from t_PDABarCodeSign_Out t1 inner join ICStockBillEntry t2 on t1.FInterID=t2.FInterID  where t1.FBarCode=@FBarCode and t1.FTypeID=21
 end
 if(@FBillNo='OK')
 begin
...
insert into #Tmp11111(FItemID,FUnitID,FQty,FStockID,FStockPlaceID,FBatchNo,FKFPeriod,FKFDate,FBillNo,FExplanation,FPrice,FOLOrderBillNo)
select top 1 FItemID,FUnitID,1,FStockID,FStockPlaceID,FBatchNo,FKFPeriod,FKFDate,FBillNo,@FBillNo,@FPrice,@FOLOrderBillNo from t_PDABarCodeSign where FBarCode=@FBarCode
 
if not exists(select 1 from #Tmp11111)
begin
insert into #Tmp11111(FExplanation)values(@FBillNo)
end
select t1.FExplanation as 说明,t1.FBillNo,t1.FItemID,t1.FUnitID,convert(float,FQty) as FQty,t1.FStockID,t1.FStockPlaceID,t1.FBatchNo,t1.FKFPeriod,t1.FKFDate,t1.FPrice as 单价,t2.FNumber as 商品编码,t2.FName as 商品名称,FOLOrderBillNo as 网上订单号 from #Tmp11111 t1 inner join t_ICItem t2 on t1.FItemID = t2.FItemID
drop table #Tmp11111
commit tran 
return;
--------------存在错误
