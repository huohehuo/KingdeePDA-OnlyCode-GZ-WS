...
if(@FScanType=0)--装箱条码
begin 
  if exists(select 1 from a_DetailsTable where FOrderID=@FOrderID) --盘点条码是否已添加
	  begin
		set @FBillNo= '该箱码正在装箱,不能重复装箱'
	  end
  if exists(select 1 from t_PDAAssemble where FBillNo=@FOrderID)
     begin
     set @FBillNo= '该箱码已完成装箱,不能重复装箱'
     end
end
else
begin --盒码
