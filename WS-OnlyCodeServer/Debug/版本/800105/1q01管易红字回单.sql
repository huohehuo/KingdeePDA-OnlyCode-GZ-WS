if (exists (select * from sys.objects where name = 'proc_PushCheckLogisticsSingle_Red'))
drop proc proc_PushCheckLogisticsSingle_Red
go
create proc proc_PushCheckLogisticsSingle_Red
(
...
begin tran declare
          ...
          set @FBillNo=dbo.getString(@mainStr,'|',2)      --物料单号
          ...
          insert into t_PDALogisticsSingle(FDate,FUserID,FBillNo,FTypeID,FDeliverBillNo,FTerraceBillNo,FCustomerName)values(@FDate,@FBillerID,@FBillNo,1,@FDeliverBillNo,@FTerraceBillNo,@FCustomerName) 
          ...
          select @FID,t1.FPDAID,t1.FOrderID,t1.FBarCode,t1.FItemID,t1.FStockID,t1.FStockPlaceID, t1.FBatchNo,  t1.FKFPeriod,  t1.FKFDate,  -t1.FQty,t2.FUnitID,t2.FStockID,t2.FStockPlaceID,t2.FRemark2,t2.FDateOutStore,t2.FUserOutStore,FIDLogistics,t2.FIsOutStore from a_DetailsTable t1 left join t_PDABarCodeSign t2 on t1.FBarCode=t2.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID
          update t_PDABarCodeSign  set  FIDLogistics=@FID,FDateOutStore=@fdate,FIsOutStore='未出库',FQtyOut =FQtyOut - t.FQty,FRemark2='管易红字生单',FUserInStore=@FBillerID,FStockID=t.FStockID,FStockPlaceID=t.FStockID  from (select a.* from a_DetailsTable a left join t_PDABarCodeSign b on a.FBarCode=b.FBarCode where FOrderID=@FOrderID and FPDAID=@FPDAID) as t where t_PDABarCodeSign.FBarCode=t.FBarCode
