if (exists (select * from sys.objects where name = 'proc_CheckBarCodeFirst_Insert'))
    drop proc proc_CheckBarCodeFirst_Insert
go
create proc proc_CheckBarCodeFirst_Insert
(
  @FOrderID varchar(255),--盘点方案ID
...
set @FBillNo='OK'
   if not exists(select 1 from a_DetailsTable where FBarCode=@FBarCode)
     ...
     insert into a_DetailsTable(FPDAID,FOrderID,FBarCode,FItemID,FStockID,FStockPlaceID, FBatchNo,  FKFPeriod,  FKFDate,  FQty,FInterID)
       values( @FPDAID,@FOrderID, @FBarCode, 0,0, 0,  '',  0,  '',  1,@FOrderID)  
       end
