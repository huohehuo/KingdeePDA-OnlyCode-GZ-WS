if (exists (select * from sys.objects where name = 'proc_CheckBarCode_Insert'))
    drop proc proc_CheckBarCode_Insert
go
create proc proc_CheckBarCode_Insert
(
...
set @FBillNo='OK'
   if not exists(select 1 from a_DetailsTable where FBarCode=@FBarCode)
     ...
     insert into a_DetailsTable(FPDAID,FOrderID,FBarCode,FItemID,FStockID,FStockPlaceID, FBatchNo,  FKFPeriod,  FKFDate,  FQty)
        select top 1  @FPDAID,@FOrderID, FBarCode, FItemID,@FStockID, @FStockPlaceID,  FBatchNo,  FKFPeriod,  FKFDate,  FQty from t_PDABarCodeCheckEntry where FBarCode=@FBarCode
       end
