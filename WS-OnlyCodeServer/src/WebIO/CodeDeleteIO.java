 ...
 *          删除制定条码的临时表数据
 */
@WebServlet(urlPatterns = "/CodeDeleteIO")
public class CodeDeleteIO extends HttpServlet {
...
//        String imie = request.getParameter("imie");
        ...
        if (null ==barcode ||"".equals(barcode)){
            response.getWriter().write(gson.toJson(new WebResponse(false,"条码为空，删除失败")));
            return;
        }
...
//                sta = conn.prepareStatement(SQL);
                sta = conn.prepareStatement("delete from a_DetailsTable where FBarCode = '"+barcode+"'");
