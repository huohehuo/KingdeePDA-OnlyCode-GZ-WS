 ...
 */
@WebServlet("/WebScanSaveUpload")
public class WebScanSaveUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public WebScanSaveUpload() {
        super();
...
//		Lg.e("获得请求3", MD5Util.convertMD5(json));
		...
		if(parameter!=null&&!parameter.equals("")){
			try {
				...
				WebUploadBean pBean = gson.fromJson(parameter, WebUploadBean.class);
				for(int i =0;i<pBean.list.size();i++){
					sta = conn.prepareStatement("exec proc_XCHXUpLoad ?,?,?,?,?,?");
