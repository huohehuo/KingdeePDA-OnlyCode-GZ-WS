 ...
 */
@WebServlet("/CodeCheckInsertForOutForRed")
public class CodeCheckInsertForOutForRed extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public CodeCheckInsertForOutForRed() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(parameter!=null&&!parameter.equals("")){
			try {
				...
				sta = conn.prepareStatement("exec proc_OutStoreBarCode_Red_Insert ?,?,?,?,?,?");

					sta.setString(1, bean.FOrderID);
					sta.setString(2, bean.FPDAID);
					sta.setString(3, bean.FBarCode);
					sta.setString(4, bean.FQty);
					sta.setString(5, bean.FStockID);
					sta.setString(6, bean.FStockPlaceID);
					rs = sta.executeQuery();
					if (rs != null) {
						while (rs.next()) {
