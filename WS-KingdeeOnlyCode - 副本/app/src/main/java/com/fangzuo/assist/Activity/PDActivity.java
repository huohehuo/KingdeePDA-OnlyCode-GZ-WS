...
public class PDActivity extends BaseActivity {
    ...
    public final static int PDACTIVITY = 123552;
...
//        postToServer(container);
            final int finalI = i;
            App.getRService().doIOAction(WebApi.PDUPLOAD, gson.toJson(pBean), new MySubscribe<CommonResponse>() {
                @Override
                public void onNext(CommonResponse commonResponse) {
                    super.onNext(commonResponse);
        ...
        ordercode++;
        share.setOrderCode(PDActivity.this,ordercode);
        btnBackorder.setClickable(true);
        LoadingUtil.dismiss();
        MediaPlayer.getInstance(mContext).ok();
        lockScan(0);
