...
public class Table4Adapter extends BaseAdapter implements View.OnClickListener {
    Context context;
//    List<T_main> main;
    ...
    public Table4Adapter(Context context, List<T_Detail> detail, List<Boolean> isCheck) {
...
//        this.main = main;
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        if (view == null) {
            view = LayoutInflater.from(context).inflate(R.layout.list_table4, null);
...
//                viewHolder.productname.setText("单据编号:" + t_mains.get(0).orderId);
                viewHolder.barCode.setText("SN码:" + detail.get(i).FBarcode);
                viewHolder.productId.setText("箱码:" + detail.get(i).DataPush);
                viewHolder.productname.setText("物料名称:" + detail.get(i).FProductName);
                viewHolder.productxh.setText("物料编码:" + detail.get(i).FProductCode);
                viewHolder.num.setText("数量:" + detail.get(i).FQuantity+detail.get(i).FUnit);
                viewHolder.date.setText("录入日期:" + detail.get(i).DataInput);
//            }else {
